...
function getDigitalRoot(num) {
  while (String(num).length > 1) {
    num = (num + '').split('').reduce((acc, mov) => acc + Number(mov), 0);
  }
  return num;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  const splitPath = pathes.map((path) => path.split('/'));

  for (let i = 0; i < splitPath[0].length; i += 1) {
    const dir = splitPath[0][i];
    if (splitPath.every((path) => path[i] === dir)) stack.push(dir);
  }
...
function getMatrixProduct(m1, m2) {
  ...
  const result = Array(rowsM1)
    .fill(null)
    .map(() => Array(colsM2).fill(0));
...
function evaluateTicTacToePosition(position) {
  const checkLine = (a, b, c) => a === b && b === c && a !== undefined;
